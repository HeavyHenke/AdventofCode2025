using System.Numerics;
using System.Text.RegularExpressions;

namespace AdventofCode2025;

public partial class Day2
{
    public string Part1()
    {
        //var input = "11-22,95-115,998-1012,1188511880-1188511890,222220-222224,\n1698522-1698528,446443-446449,38593856-38593862,565653-565659,\n824824821-824824827,2121212118-2121212124";
        var input = "52500467-52574194,655624494-655688785,551225-576932,8418349387-8418411293,678-1464,33-79,74691-118637,8787869169-8787890635,9898977468-9899009083,548472423-548598890,337245835-337375280,482823-543075,926266-991539,1642682920-1642753675,3834997-3940764,1519-2653,39697698-39890329,3-21,3251796-3429874,3467-9298,26220798-26290827,80-124,200638-280634,666386-710754,21329-64315,250-528,9202893-9264498,819775-903385,292490-356024,22-32,2663033-2791382,133-239,56514707-56704320,432810-458773,4949427889-4949576808";
        BigInteger sum = 0;

        var parts = input.Split(',');
        foreach (var part in parts)
        {
            var range = part.Split('-');
            var start = BigInteger.Parse(range[0]);
            var end = BigInteger.Parse(range[1]);

            while (start <= end)
            {
                var str = start.ToString();
                if (str.Length % 2 == 0)
                {
                    var half = str.Substring(0, (str.Length + 1) / 2);
                    if (str == half + half)
                        sum += start;
                }
                
                start++;
            }
        }
        
        return sum.ToString();
    }

    public string Part2()
    {
        //var input = "11-22,95-115,998-1012,1188511880-1188511890,222220-222224,\n1698522-1698528,446443-446449,38593856-38593862,565653-565659,\n824824821-824824827,2121212118-2121212124";
        var input = "52500467-52574194,655624494-655688785,551225-576932,8418349387-8418411293,678-1464,33-79,74691-118637,8787869169-8787890635,9898977468-9899009083,548472423-548598890,337245835-337375280,482823-543075,926266-991539,1642682920-1642753675,3834997-3940764,1519-2653,39697698-39890329,3-21,3251796-3429874,3467-9298,26220798-26290827,80-124,200638-280634,666386-710754,21329-64315,250-528,9202893-9264498,819775-903385,292490-356024,22-32,2663033-2791382,133-239,56514707-56704320,432810-458773,4949427889-4949576808";
        BigInteger sum = 0;

        var regex = MyRegex();
        var parts = input.Split(',');
        foreach (var part in parts)
        {
            var range = part.Split('-');
            var start = BigInteger.Parse(range[0]);
            var end = BigInteger.Parse(range[1]);

            while (start <= end)
            {
                var str = start.ToString();
                if (regex.IsMatch(str))
                    sum += start;
                
                start++;
            }
        }
        
        return sum.ToString();
    }

    [GeneratedRegex(@"^(\d+)\1+$")]
    private static partial Regex MyRegex();
}